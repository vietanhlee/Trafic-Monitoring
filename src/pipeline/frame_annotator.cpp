...
bool IsPointAboveGateLine(const cv::Point& p, const cv::Point& p1, const cv::Point& p2) {
	// Chuẩn hóa hướng line theo trục x để quy ước "phía trên" ổn định.
	cv::Point a = p1;
	cv::Point b = p2;
	...
	if (dx == 0) {
		// Trường hợp line đứng: quy ước "trên" là y nhỏ hơn điểm cao hơn của line.
	...
	// Hệ tọa độ ảnh: y hướng xuống dưới, nên cross < 0 tương ứng nằm phía trên line.
	return cross < 0;
}
...
// Vẽ bounding box biển số + nhãn text OCR và độ tin cậy.
...
// Vẽ bounding box phương tiện + thông tin track, brand và plate đã chấp nhận.
void DrawVehicle(
	cv::Mat& bgr,
...
struct PlatePipelineResult {
	// Kết quả OCR theo từng plate candidate hợp lệ.
	std::vector<ocr_batch::OcrText> texts;
	// BBox biển số đã map về hệ tọa độ ảnh gốc.
	std::vector<yolo_detector::Detection> plate_boxes_in_image;
	// Chỉ số xe cha ứng với mỗi plate (index trong input vehicle_crops).
	std::vector<size_t> plate_vehicle_indices;
	// Có/không có plate detect cho từng xe trong tập input.
	std::vector<bool> vehicle_has_plate;
	// Đánh dấu nhánh có ít nhất 1 biển số hợp lệ hay không.
	bool has_any_plate = false;
};

// Chạy nhánh detect plate + OCR trên danh sách crop xe, trả về kết quả đã map về ảnh gốc.
PlatePipelineResult RunPlatePipeline(
	...
	// 1) Detect biển số trong từng crop phương tiện (đa luồng ở plate_parallel).
	...
	// 2) Lọc box biển số hợp lệ và quy đổi về candidate OCR.
	...
	auto map_work = [&]() {
		// Map box biển số từ tọa độ trong crop xe về tọa độ trên ảnh gốc.
		// mapped: danh sách plate boxes trên khung hình gốc.
		std::vector<yolo_detector::Detection> mapped;
		// mapped_vehicle_indices: quan hệ plate -> vehicle index của tập input.
		std::vector<size_t> mapped_vehicle_indices;
		mapped.reserve(candidates.size());
		mapped_vehicle_indices.reserve(candidates.size());
		// has_plate đánh dấu nhánh xe nào detect được plate.
...
// Khởi tạo context tracking (ByteTrack + identity store) với cấu hình từ app_config.
...
// Vẽ FPS hiện tại lên góc trái frame.
...
// Infer 1 frame và đóng gói kết quả vào overlay để có thể draw/caching.
bool InferFrameOverlay(
	...
	if (tracking_ctx != nullptr) {
		// Tracking cấp ID bền vững theo frame, giúp hợp nhất plate/brand theo thời gian.
		track_ids = tracking_ctx->tracker.Update(vehicles_used);
	}

	// Gate runtime: cho mỗi xe, xác định có được phép chạy nhánh predict hay không.
	...
	if (tracking_ctx != nullptr && tracking_ctx->enable_predict_on_line_cross) {
		for (size_t i = 0; i < vehicles_used.size(); ++i) {
			...
			// Chỉ check một phía của line: dùng điểm đáy-trái bbox ở frame hiện tại.
	...
	// need_brand_indices/crops: tập con xe cần classify brand ở frame này.
	std::vector<size_t> need_brand_indices;
	std::vector<cv::Mat> need_brand_crops;
	// need_plate_indices/crops/rects: tập con xe cần detect plate + OCR.
	...
	for (size_t i = 0; i < vehicles_used.size(); ++i) {
		...
		if (vehicles_used[i].cls == 0) {
			// Brand chỉ áp dụng cho car và bỏ qua nếu track đã có kết quả accepted.
		...
		if (allow_predict && !has_plate) {
			// Chỉ infer plate/OCR cho track chưa có biển số accepted để tiết kiệm tài nguyên.
	...
	if (!need_brand_crops.empty() && !need_plate_crops.empty()) {
		// Chạy song song 2 nhánh độc lập để rút ngắn độ trễ frame:
	...
	// brand_id_per_vehicle lưu kết quả tạm theo toàn bộ vehicles_used.
	...
	for (size_t k = 0; k < need_brand_indices.size() && k < car_brand_results.size(); ++k) {
		...
		// Map kết quả classify về đúng vị trí xe ban đầu.
	...
	// vehicle_has_plate là map kết quả detect plate theo toàn bộ vehicles_used.
	...
	for (size_t sub_idx = 0; sub_idx < need_plate_indices.size(); ++sub_idx) {
		...
		if (sub_idx < plate_result.vehicle_has_plate.size()) {
			// Map bool has_plate từ index subset về index toàn bộ vehicles_used.
			vehicle_has_plate[global_idx] = plate_result.vehicle_has_plate[sub_idx];
		}
		if (tracking_ctx != nullptr && !vehicle_has_plate[global_idx]) {
			// Không detect được plate ở frame này => vẫn tính 1 lần thử cho track.
	...
	for (size_t plate_idx = 0; plate_idx < plate_result.texts.size(); ++plate_idx) {
		...
		if (tracking_ctx != nullptr) {
			// Lưu tích lũy kết quả OCR vào identity store để chấp nhận theo nhiều lần đo.
			// Lưu ý: update theo track_id của xe cha sau khi map subset -> global index.
	...
	for (size_t i = 0; i < vehicles_used.size(); ++i) {
		...
		// Luôn hiện ID từ đầu nếu đang bật tracking; thuộc tính (biển số/hãng) sẽ cập nhật dần.
		...
		if (iden != nullptr && iden->plate_accepted) {
			// Hiện text đã chấp nhận (ổn định hơn text OCR từng frame).
			v.accepted_plate_text = iden->plate_text;
		}
		out_overlay.vehicles.push_back(v);
	}

	// Cập nhật trace (lịch sử tâm bbox) theo track_id và đưa vào overlay để có thể cache/draw lại.
	if (tracking_ctx != nullptr) {
	...
	for (size_t i = 0; i < plate_result.texts.size(); ++i) {
		...
		// Track ID gán theo xe cha để truy vết trên UI/log.
...
// Vẽ toàn bộ overlay (trace, xe, biển số) lên frame.
...
// API mức cao: infer + vẽ trực tiếp lên frame đầu vào.
bool AnnotateFrame(
	cv::Mat& bgr,
	Ort::Session& vehicle_sess,
	Ort::Session& plate_sess,
